...
- "menu?" → short category overview (mark breakfast "selected branches only"), then the URL.
- Specific category (pizza, hot pot, kids meal, snacks, …) → list **every item** in that category with item code and price, no matter how many. Do not shorten to representative items or ask whether they want the rest.
