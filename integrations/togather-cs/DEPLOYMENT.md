...
- **会员 App QR 码图片**：✅ 已接入。引擎新增 bot 出站图片能力（`internal/im/bot_media.go`）：媒体平台（WhatsApp）上，bot 最终回复里的 markdown 图片 `![alt](https://…)` 会被服务端下载（SSRF 白名单校验、仅 image/*、单张 16 MiB 上限、每条回复最多 3 张）并作为真实图片消息发出，对应 markdown 从文本中移除；下载失败则保留原文本兜底。两张 QR PNG 放在 `integrations/togather-distance/assets/`，由 app 的公开静态路由 `GET /api/v1/integration-assets/togather-distance/<file>`（`internal/integrationassets`，段校验 + 扩展名白名单）提供；system prompt 用**相对路径**输出图片 markdown，因此 Web 控制台（同源经 frontend nginx 代理）与 WhatsApp（引擎识别该前缀直接读本地文件）都能显示。togather-distance 容器的 `GET /assets/<name>` 路由保留作备用。
