export default {
  ...
  agentEditor: {
    ...
    audioUpload: {
      ...
      desc: 'Позволяет пользователям загружать аудиофайлы в чате; голосовые сообщения из IM-каналов (например, WhatsApp) также транскрибируются. Система автоматически транскрибирует их с помощью ASR-модели.',
      asrModel: 'ASR-модель',
    ...
    im: {
      ...
      whatsappScanBind: 'Сканировать QR для сопряжения WhatsApp',
      whatsappScanning: 'Откройте WhatsApp на телефоне → Настройки → Связанные устройства → Привязка устройства и отсканируйте этот код',
      whatsappBindSuccess: 'WhatsApp успешно сопряжён',
      whatsappPairedUnsaved: 'Сопряжено — сохраните, чтобы применить',
      whatsappRebind: 'Повторное сопряжение',
      whatsappHint: 'Подключает обычный аккаунт WhatsApp как сопутствующее устройство по мультиустройственному протоколу WhatsApp Web (QR-сопряжение, без официального Cloud API).',
      whatsappRiskWarning: 'Неофициальная интеграция: автоматизация личного аккаунта нарушает условия использования WhatsApp и несёт реальный риск блокировки аккаунта. Используйте отдельный номер.',
      whatsappQRExpired: 'QR-код истёк, нажмите для обновления',
      whatsappAllowFrom: 'Список разрешённых для ЛС',
      whatsappAllowFromHint: 'Номера телефонов через запятую (с кодом страны, напр. 8613800138000), которым разрешено обращаться к боту в личных сообщениях; «*» — разрешить всем, пусто — отклонять все ЛС. В группах бот отвечает при {\'@\'}упоминании или при ответе на его сообщение.',
      statusConnected: 'Подключено',
      statusConnecting: 'Подключение…',
      statusLoggedOut: 'Сопряжение отозвано — требуется повторное сопряжение',
      statusNeedsPairing: 'Не сопряжено — отсканируйте QR-код',
      statusStreamReplaced: 'Сессию перехватил другой клиент',
      statusNotRunning: 'Не запущено',
      ...
      handoff: {
        section: 'Перевод на оператора',
        enable: 'Автоматический перевод',
        enableHint: 'Передавать диалог оператору и отправлять уведомление, когда сообщение содержит ключевое слово или бот несколько раз подряд не смог ответить.',
        keywords: 'Ключевые слова',
        keywordsPlaceholder: 'оператор, живой человек, поддержка',
        keywordsHint: 'Срабатывает, если сообщение пользователя содержит любое из слов (без учёта регистра); разделяйте запятыми. Избегайте слишком общих слов.',
        fallbackThreshold: 'Порог неотвеченных подряд',
        fallbackThresholdHint: 'Срабатывает после стольких подряд ответов бота без результата (ошибка или пустой ответ); 0 — триггер выключен.',
        autoReply: 'Сообщение о переводе',
        autoReplyPlaceholder: 'Переводим вас на оператора — он ответит, как только увидит обращение.',
        timeout: 'Окно передачи (минуты)',
        timeoutHint: 'Сколько бот молчит после срабатывания; по окончании окна он снова отвечает автоматически.',
        noTakeoverHint: 'На этой платформе нет ответов из консоли: триггер только отправляет уведомление и сообщение пользователю, бот продолжает отвечать.',
        webhook: 'Webhook для уведомлений',
        webhookHint: 'POST-запрос при каждом срабатывании; подходят webhook-адреса ботов WeCom/DingTalk/Feishu или Slack.',
        webhookFormat: 'Формат уведомления',
        formatGeneric: 'Общий JSON',
        formatWecom: 'Бот WeCom',
        formatDingtalk: 'Бот DingTalk',
        formatFeishu: 'Бот Feishu',
        formatSlack: 'Slack',
        needTrigger: 'При включённом автопереводе настройте хотя бы ключевые слова или порог неотвеченных подряд.',
        badWebhook: 'Webhook для уведомлений должен быть http(s)-адресом.'
      }
    },
    embed: {
  ...
  model: {
    ...
    editor: {
      ...
      codex: {
        ...
        oauthLabel: 'Вход через ChatGPT',
        oauthButton: 'Войти через аккаунт ChatGPT',
        oauthButtonAgain: 'Повторная авторизация',
        oauthWaiting: 'Страница авторизации открыта — завершите вход в ChatGPT во всплывающем окне…',
        oauthSuccess: 'Авторизовано: {account} — токены заполнены автоматически',
        oauthSuccessSaved: 'Авторизовано: {account} — учётные данные сохранены в этой модели',
        oauthSuccessNoProfile: 'Авторизация выполнена — токены заполнены автоматически',
        oauthCredentialSaved: 'Учётные данные обновлены (access + refresh token)',
        oauthFailed: 'Авторизация не удалась, попробуйте ещё раз',
        oauthExpired: 'Попытка входа истекла — нажмите кнопку ещё раз',
        oauthTimeout: 'Время ожидания авторизации истекло — нажмите кнопку ещё раз',
        oauthListenerDown: 'Бэкенд не смог открыть порт localhost:1455 (порт занят?). После входа браузер остановится на недоступной странице localhost:1455 — вставьте её полный URL в поле выше.',
        oauthHint: 'Откроется auth.openai.com для входа через ChatGPT; токены заполняются автоматически и ротируются при истечении. Если браузер не на той же машине, что WeKnora, вставьте итоговый адрес localhost:1455 в появившееся поле.',
        oauthPastePlaceholder: 'Переход на localhost:1455 не открывается? Вставьте сюда полный URL из адресной строки',
        oauthPasteSubmit: 'Отправить',
        ...
        authJsonLabel: 'Или импортируйте вручную из ~/.codex/auth.json',
  ...
  chat: {
    ...
    channelApi: 'API',
    channelIm: 'IM',
    channelImTakeover: 'Во время перехвата',
    imManualBanner: 'Диалог из {platform}: отправленный здесь текст уйдёт собеседнику напрямую как ответ оператора, без ответа ИИ',
    imManualSent: 'Отправлено собеседнику',
    imManualSendFailed: 'Не удалось отправить ответ оператора',
    imManualTag: 'Ответ оператора',
    imManualImageNoPreview: 'Изображение {name} отправлено (слишком большое, предпросмотр не сохранён)',
    imTakeoverStart: 'Перехватить диалог',
    imTakeoverRelease: 'Вернуть боту',
    imTakeoverActive: 'Диалог у оператора: бот молчит, отвечаете вы',
    imTakeoverActiveUntil: 'Диалог у оператора: бот молчит и вернётся автоматически в {time}, если оператор не ответит',
    imTakeoverFor30m: 'Перехватить на 30 минут',
    imTakeoverFor1h: 'Перехватить на 1 час',
    imTakeoverFor4h: 'Перехватить на 4 часа',
    imTakeoverIndefinite: 'Перехватить до ручного возврата',
    imTakeoverStarted: 'Диалог перехвачен — бот молчит',
    imTakeoverReleased: 'Диалог возвращён боту',
    imTakeoverFailed: 'Не удалось обновить состояние перехвата',
  ...
  input: {
    ...
    placeholderManualIm: 'Введите ответ оператора — собеседник получит его напрямую',
  ...
  // Входящие оператора: рабочее место поддержки в IM
  imInbox: {
    title: 'Входящие IM',
    unreadTotalTip: 'Всего непрочитанных сообщений',
    filterAll: 'Все',
    filterHuman: 'Нужен оператор',
    filterUnread: 'Непрочитанные',
    empty: 'Пока нет IM-диалогов',
    emptyFiltered: 'Нет диалогов по этому фильтру',
    loading: 'Загрузка…',
    loadMore: 'Загрузить ещё',
    loadOlder: 'Загрузить более ранние сообщения',
    selectPrompt: 'Выберите диалог слева, чтобы начать работу',
    humanTag: 'Перехвачен оператором',
    roleUser: 'Пользователь',
    roleAssistant: 'Бот',
    roleOperator: 'Оператор',
    takeover: 'Перехватить',
    takeover30m: 'Перехватить на 30 минут',
    takeover1h: 'Перехватить на 1 час',
    takeover4h: 'Перехватить на 4 часа',
    takeoverNoExpiry: 'Перехватить до ручного возврата',
    takeoverActive: 'Перехвачен оператором, истекает в {time}',
    takeoverActiveNoExpiry: 'Перехвачен оператором',
    takeoverStarted: 'Диалог перехвачен; бот замолчал',
    takeoverReleased: 'Возвращено боту',
    takeoverRelease: 'Вернуть боту',
    openInChat: 'Открыть в чате',
    composerPlaceholder: 'Введите ответ. Enter — отправить, Shift+Enter — новая строка',
    composerUnsupported: 'Для этой платформы ответы из консоли пока не поддерживаются; ответьте в самом мессенджере',
    send: 'Отправить',
    ...
    quickReplies: 'Быстрые фразы',
    quickRepliesManage: 'Управление быстрыми фразами',
    quickRepliesHint: 'По одной фразе в строке, до 50 фраз по 500 символов; общие для всех операторов пространства.',
    quickRepliesPlaceholder: 'Например: Здравствуйте! Чем могу помочь?',
    quickRepliesSaved: 'Быстрые фразы сохранены',
    save: 'Сохранить',
    cancel: 'Отмена',
    channelTakeover: 'Во время перехвата оператором',
    channelManual: 'Оператор',
  },
  menu: {
    knowledgeBase: 'База знаний',
    agents: 'Агенты',
