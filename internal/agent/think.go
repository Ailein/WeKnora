...
func (e *AgentEngine) callLLMWithRetry(
	...
	// retry once so unsupported providers degrade to normal behavior. Overflow
	// errors are excluded: the compaction retry above owns them, and dropping
	// the forcing would not change the request size.
	if err != nil && !isTransientError(err) && !compaction.IsOverflowError(err) &&
